...
int main()
{
	...
	Dist = farthest(n, P);
	bestDist = n * Dist; // Initialize bestDist as largest possible dist * n
...
void print_perm(int n, int *A, int sizeA, point2D *P, int *bestSet, float &bestDist)
{
	...
	if (n == 1)
	{
		...
		cout << "Permutation to compare: " << endl;
		print_cycle(sizeA, P, A);

		// Calculate the permutation's/Hamiltonian cycle distance
		for (i = 0; i < sizeA; i++)
		{
			if (i == (sizeA - 1))
				dist += sqrt(pow((P[A[0]].x - P[A[sizeA - 1]].x), 2) + pow((P[A[0]].y - P[A[sizeA - 1]].y), 2));
			else
				dist += sqrt(pow((P[A[i]].x - P[A[i + 1]].x), 2) + pow((P[A[i]].y - P[A[i + 1]].y), 2));
		}
		
		cout << "Current cycle's distance: " << dist << endl;
		...
		if (dist < bestDist)
		{
			...
			bestSet = A;
			bestDist = dist;
			cout << "New best set is now... " << endl;
			print_cycle(sizeA, P, bestSet);
			cout << "New best distance is now: ";
			cout << bestDist << endl;
		}
